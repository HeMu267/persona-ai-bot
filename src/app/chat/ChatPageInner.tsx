...
export default function ChatPageInner({ personaKey }: { personaKey: string }) {
  const currentPersona = personaDetails[personaKey as keyof typeof personaDetails] 
    || personaDetails.hiteshPersona;
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { sender: "bot", text: data?.reply || "Sorry, I received an invalid response." },
      ]);
    } catch (err) {
      console.error(err);
      setMessages((prev) => [
